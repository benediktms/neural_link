...
fn generate_docs_content() -> String {
  string.join(
    [
      ...
      "### Persistence",
      "",
      "neural_link persists room events to a local SQLite database under `~/.neural_link/` by default (`neural_link.db` and `sync.jsonl`). Set `NEURAL_LINK_DATA_DIR` to override that location. If neither `NEURAL_LINK_DATA_DIR` nor `HOME` is available, path resolution fails. SQLite writes are best-effort (fire-and-forget) — errors are logged but never fail MCP responses. Actors remain the source of truth at runtime; SQLite provides durability.",
      "",
      ...
      "",
      "```",
      "nlk sync [--db ~/.neural_link/neural_link.db] [--log ~/.neural_link/sync.jsonl] [--brain name]",
      "```",
      "",
      "Sync reads closed rooms from SQLite, builds a conversation transcript, and calls `brain artifacts create` for each unsynced room. Progress is tracked in a JSONL log at `~/.neural_link/sync.jsonl` (decoupled from SQLite). Re-running is idempotent — already-synced rooms are skipped.",
      "",
      "### Message kinds",
      "",
...
fn generate_tools_doc() -> String {
  let tools = [
    #(
      "room_open",
      "Create a coordination room. The opener is auto-joined as the room's lead. The server always assigns a fresh random room id. The `brains` param configures persistence plugins. For cross-process reconnect, set `external_ref` and look up later via `room_find_by_external_ref`",
    ...
    #(
      ...
      "Look up rooms by the `external_ref` they were opened with. Returns matching rooms with their status (open, closed, stale). Use to re-attach across process restarts: the replacement supplies the same external_ref and discovers the prior room id",
