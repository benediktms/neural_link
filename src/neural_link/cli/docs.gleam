...
fn generate_docs_content() -> String {
  string.join(
    [
      ...
      "|------|-----|------------------|",
      "| **Lead** | The session that dispatches subagents | Opens the room (auto-joined as lead). Monitors messages, answers questions, responds to escalations, unblocks agents. Cannot leave — only closes the room. Persists the summary. |",
      "| **Subagent** | Each dispatched agent | Joins the room on activation. Communicates findings, blockers, and questions. Reads inbox periodically. Calls `room_leave` when done (replaces silent completion). |",
      "",
      "### Coordination flow",
      "",
      "#### Lead (dispatcher)",
      "",
      "1. **Open a room** — call `room_open` with `title`, `purpose`, `participant_id`, and `display_name`. The opener is auto-joined as the room's lead. No separate `room_join` needed. If working with brain-tracked projects, pass `brains` to enable artifact persistence on close.",
      ...
      "3. **Monitor and participate** — while subagents work, the lead periodically reads the room via `inbox_read`. Answer questions from subagents. Ask questions if something is unclear. Send `decision` messages to resolve ambiguities. Unblock agents that report `blocker` messages. Respond to `escalation` messages when subagents depart while others depend on them.",
      "4. **Close the room** — once all subagents have sent `handoff` and called `room_leave`, call `room_close` with a resolution. The server persists the full conversation as a brain artifact (if brains were declared) and returns structured extraction data: decisions, open questions, blockers, participant list, message count, and artifact record ID.",
      ...
      "4. **Send `handoff` then `room_leave` before returning.** If you are a subagent and your work is done, send a handoff message then call `room_leave`. Silent completion causes deadlocks.",
      ...
      "11. **Call `room_leave` before returning.** Subagents must call `room_leave` after sending `handoff`. The system drains outbound obligations before departing. Skipping `room_leave` can strand waiting agents.",
      "12. **Respond to `escalation` messages immediately.** The lead receives escalations when departing agents leave behind unresolved waits. Ignoring escalations deadlocks waiting agents.",
    ],
    "\n",
  )
}
...
fn generate_tools_doc() -> String {
  let tools = [
    #(
      "room_open",
      "Create a coordination room. The opener is auto-joined as the room's lead",
